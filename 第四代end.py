...
def ai_recognize(img_file):
    """
    识别图片内容
    :param img_file: UploadedFile 对象或文件路径字符串
    :return: 识别结果字符串
    """
    if not img_file:
        return "无图片"
    ...
    try:
        # 【修改】兼容处理：支持 UploadedFile 对象和文件路径
        # 在公网部署时，st.file_uploader 返回的是 UploadedFile，不能直接当作路径打开
        byte_data = None
        
        if hasattr(img_file, 'read'):
            # 如果是 UploadedFile 对象，读取字节流
            # 注意：UploadedFile 的 read 指针可能需要重置，但通常第一次读取是有效的
            # 为了安全，我们读取一次并保存
            original_pos = img_file.tell() if hasattr(img_file, 'tell') else 0
            byte_data = img_file.read()
            # 如果后续还需要使用该文件对象，可能需要 seek(0)，但这里只用于识别
        ...
        try:
            ...
            return f"图片格式错误: {str(img_err)[:20]}"
        ...
        if "error_code" in result_json:
            return f"API错误: {result_json.get('error_msg')}"
...
def edit_item_logic(item_id, name, category, time_val, location, description, contact, imgs=None, remarks=None):
    ...
    try:
        ...
        if imgs:
            try:
                # 【修改】传递第一个上传文件对象给 ai_recognize，它现在能处理 UploadedFile
            ...
            for img_file in imgs:
                try:
                    # 【修改】安全地处理 UploadedFile 对象转换为 Image
                    # 读取字节流
...
def main_page():
    ...
    # --- 一键导入 ---
        ...
        st.info("💡 提示：若需导入图片，请在Excel中增加一列‘图片路径’，填写**服务器端**的绝对路径。在公网部署时，本地路径无效。")
        ...
        if f and st.button("开始导入", key="btn_start_import"):
            try:
                df = pd.read_excel(f)
                cnt = 0
                err = 0
                ...
                # 检查是否有图片路径列
                has_img_col = '图片路径' in df.columns or 'image_path' in df.columns or 'img' in df.columns
                img_col_name = None
                ...
                for _, r in df.iterrows():
                    try:
                        # 假设 Excel 列顺序: 名称, 时间, 地点, 描述... 
                        # 为了更健壮，建议通过列名获取，如果列名不存在则回退到iloc
                        name = str(r.get('物品名称', r.iloc[0])) if (pd.notna(r.get('物品名称', r.iloc[0])) if '物品名称' in r else pd.notna(r.iloc[0])) else ""
                        
                        # 尝试智能获取列
                        loc_val = r.get('地点', r.iloc[2]) if ('地点' in r or len(r) > 2) else ""
                        loc = str(loc_val) if pd.notna(loc_val) else ""
                        
                        time_val = r.get('时间', r.iloc[1]) if ('时间' in r or len(r) > 1) else ""
                        t_str = str(time_val) if pd.notna(time_val) else ""
                        
                        desc_val = r.get('描述', "") 
                        desc = str(desc_val) if pd.notna(desc_val) else ""
                        ...
                        if has_img_col and img_col_name:
                            img_path = r.get(img_col_name)
                            if pd.notna(img_path) and str(img_path).strip():
                                path_str = str(img_path).strip()
                                # 尝试读取图片
                                try:
                                    # 【修改】在公网部署时，必须使用服务器上的绝对路径
                                    # 如果是相对路径，尝试结合当前目录（仅适用于本地部署或特定结构）
                                    ...
                                    if os.path.exists(full_path):
                                        with Image.open(full_path) as img:
                                            ...
                                            if b64:
                                                ...
                                                # 简单AI识别（可选，耗时较长，这里暂跳过以加快导入速度，或仅保存）
                                                # ai_res = ai_recognize(open(full_path, 'rb')) 
                                                ai_res = "已导入图片"
                                                img_cnt += 1
                                    else:
                                        # 公网部署时常见情况：路径不存在，跳过图片但不中断导入
                                        pass 
                                except Exception as e:
                                    pass # 图片读取失败忽略
                        ...
                        c.execute('''INSERT INTO items (item_no,type,name,category,time,location,description,image,ai_result,publisher,contact,create_time,status,is_hidden,remarks)
                                     VALUES (?,"lost",?,"其他",?,?,?,?,"","",?,0,0,"")''',
                                  (ino, name, t_str, loc, desc, image_json, ai_res, str(datetime.datetime.now())))
                        cnt +=1
                    except Exception as e:
                        err += 1
                        continue
                ...
                conn.commit()
                conn.close()
                st.success(f"导入完成！成功：{cnt} 条，跳过/失败：{err} 条，其中包含图片：{img_cnt} 张")
            except Exception as e:
