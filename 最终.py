...
import sqlite3
import pandas as pd
import difflib  # 新增：用于字符串相似度匹配

# ===================== 配置 =====================
# 修改：从环境变量读取 API Key，避免硬编码泄露，同时兼容本地测试
BAIDU_API_KEY = os.getenv("BAIDU_API_KEY", "无")
BAIDU_SECRET_KEY = os.getenv("BAIDU_SECRET_KEY", "无")

# 修改：确定数据库路径，确保在云端也能正确访问
# 使用绝对路径或基于当前脚本所在目录的路径，避免工作目录变化导致的问题
...
# ===================== 新增：物品编辑逻辑 =====================
def edit_item_logic(conn, item_id, name, category, time_val, location, description, contact, imgs=None):
    """
    通用物品编辑逻辑，支持普通用户和管理员
    """
    c = conn.cursor()
    ...
    # 获取原物品信息以保留未修改的图片
    c.execute("SELECT image, ai_result FROM items WHERE id=?", (item_id,))
    old_data = c.fetchone()
    old_image_json = old_data[0] if old_data else None
    old_ai = old_data[1] if old_data else "未识别"
    
    new_image_json = old_image_json
    new_ai = old_ai
    
    # 如果上传了新图片，则重新处理图片和AI识别
    if imgs:
        try:
            # 使用第一张图片进行AI识别
            new_ai = ai_recognize(imgs[0])
        except:
            new_ai = "识别失败"
        ...
        if b64_list:
            ...
            # 如果上传了但处理失败，保持原样或清空？这里选择保持原样以防误操作
            pass

    c.execute('''UPDATE items SET name=?, category=?, time=?, location=?, description=?, contact=?, image=?, ai_result=? 
                 WHERE id=?''',
              (name, category, str(time_val), location, description, contact, new_image_json, new_ai, item_id))
    conn.commit()
    return True
...
def export_to_excel(items, filename="失物招领数据.xlsx"):
    try:
        ...
        # 修改：不再抛出异常中断程序，而是返回 None 并打印警告
        # 建议用户在 requirements.txt 中添加 openpyxl
        print("警告: 未安装 openpyxl 库，无法导出 Excel。请运行 'pip install openpyxl' 或在 requirements.txt 中添加该依赖。")
        return None
...
def get_ai_matches(user_items, all_items, col_index):
    """
    修复：优化AI匹配算法，引入相似度计算和权重排序，提高匹配精准度
    """
    matches = []
    
    # 获取动态索引
    idx_id = col_index.get('id', 0)
    idx_type = col_index.get('type', 2) # 注意：原代码 type 是第2列，但 item_no 插入后可能变动，需依赖 col_index
    ...
    # 预处理所有物品，构建快速查找字典 (ID -> Item)
    # 同时过滤掉已隐藏或已解决的物品，减少计算量
    valid_items_map = {}
    for item in all_items:
        ...
        if not ihide and istat == 0:
            valid_items_map[iid] = item
    ...
    for my_item in user_items:
        ...
        my_ai_res = my_item[idx_ai] if idx_ai < len(my_item) else ""
        
        scored_candidates = []

        # 遍历所有有效的相反类型物品
        for mid, m_item in valid_items_map.items():
            if mid == iid:
                continue
                
            m_type = m_item[idx_type]
            if m_type != oppo_type:
                continue
                
            m_name = m_item[idx_name]
            m_ai_res = m_item[idx_ai] if idx_ai < len(m_item) else ""
            ...
            # --- 策略1：名称相似度匹配 (高权重) ---
            # 使用 difflib 计算名称相似度
            name_ratio = difflib.SequenceMatcher(None, iname, m_name).ratio()
            
            if name_ratio > 0.85:
                # 极高相似度（几乎完全一致）
                score += 50.0
                match_reasons.append(f"名称高度相似 ({int(name_ratio*100)}%)")
            elif name_ratio > 0.6:
                # 中等相似度
                score += 20.0
                match_reasons.append(f"名称部分相似 ({int(name_ratio*100)}%)")
            
            # --- 策略2：AI特征与描述的综合匹配 (中权重) ---
            # 将AI结果清洗，去除常见无意义词
            def clean_text(text):
                if not text or text in ["未识别", "无图片", "识别失败", "未上传"]:
                    return ""
                return text.replace("、", " ").replace(",", " ")

            my_ai_clean = clean_text(my_ai_res)
            m_ai_clean = clean_text(m_ai_res)
            
            ai_score = 0.0
            
            if my_ai_clean and m_ai_clean:
                # 计算 AI 文本之间的相似度
                ai_ratio = difflib.SequenceMatcher(None, my_ai_clean, m_ai_clean).ratio()
                if ai_ratio > 0.5:
                    ai_score += 15.0 * ai_ratio
                    match_reasons.append(f"AI特征匹配 ({int(ai_ratio*100)}%)")
                
                # 交叉匹配：我的AI vs 对方名称
                cross_ratio_1 = difflib.SequenceMatcher(None, my_ai_clean, m_name).ratio()
                if cross_ratio_1 > 0.6:
                    ai_score += 10.0 * cross_ratio_1
                    
                # 交叉匹配：对方AI vs 我的名称
                cross_ratio_2 = difflib.SequenceMatcher(None, m_ai_clean, iname).ratio()
                if cross_ratio_2 > 0.6:
                    ai_score += 10.0 * cross_ratio_2
                    
            elif my_ai_clean and not m_ai_clean:
                 # 只有我有AI，尝试匹配对方名称
                 cross_ratio = difflib.SequenceMatcher(None, my_ai_clean, m_name).ratio()
                 if cross_ratio > 0.6:
                     ai_score += 10.0 * cross_ratio
            ...
            my_cat = my_item[idx_cat] if idx_cat < len(my_item) else ""
            m_cat = m_item[idx_cat] if idx_cat < len(m_item) else ""
            if my_cat and m_cat and my_cat == m_cat:
                score += 5.0

            # 设定最低门槛，避免无关推荐
            if score > 10.0:
                scored_candidates.append((score, m_item, match_reasons))

        # 按分数降序排序
        scored_candidates.sort(key=lambda x: x[0], reverse=True)
        
        # 提取前10个最佳匹配
        final_matches = [item for _, item, _ in scored_candidates[:10]]
        
        # 为了在UI上显示匹配原因，我们可以将原因存储在 session_state 或者简单地截断
        # 这里为了保持函数纯度和兼容性，只返回物品列表。
        # 如果需要显示匹配原因，建议修改返回值结构，但考虑到UI改动较大，此处仅优化排序和筛选。
...
def login_page():
    ...
    with tab1:
        ...
        if st.button("登录", key="login_btn", use_container_width=True):
            ...
            conn.close()
            if res:
                # 修复：安全获取 is_active，防止旧数据缺失该列导致索引越界
                is_active = res[2] if len(res) > 2 else 1
                if is_active == 0:
                    st.error("该账号已被管理员禁用")
...
def main_page():
    ...
    <div style="text-align: center; font-size: 10px; color: #999; padding: 10px 0;">
        <p style="margin: 2px 0;">项目组长兼程序设计：罗元茜</p>
        <p style="margin: 2px 0;">组员：李嘉艺，冯时千，徐昕翘</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if choice == "🏠 首页":
        ...
        # 定义渲染物品卡片的辅助函数，减少重复代码
        def render_my_item_card(it, is_active_tab=True):
            idx_item_no = col_index.get('item_no', 1)
            it_no = it[idx_item_no] if len(it) > idx_item_no and it[idx_item_no] else f"ID:{it[0]}"
            item_id = it[col_index.get('id', 0)]
            
            # 唯一Key前缀
            key_prefix = f"my_item_{item_id}"
            ...
            with st.expander(f"{'🔴' if it[1]=='lost' else '🟢'} [{it_no}] {it[2]}"):
                ...
                if is_active_tab:
                    ...
                    # 检查是否正在编辑此物品
                    editing_id = st.session_state.get("editing_my_item_id")
                    
                    if editing_id != item_id:
                        # 显示操作按钮
                        btn_col1, btn_col2, btn_col3 = st.columns(3)
                        with btn_col1:
                            if it[idx_stat]==0 and st.button("✅ 标记已解决", key=f"d{item_id}", use_container_width=True):
                                c.execute("UPDATE items SET status=1 WHERE id=?", (item_id,))
                                conn.commit()
                                st.rerun()
                        with btn_col2:
                            if st.button("✏️ 编辑信息", key=f"edit_btn_{item_id}", use_container_width=True):
                                st.session_state["editing_my_item_id"] = item_id
                                st.rerun()
                        with btn_col3:
                            if st.button("🗑️ 删除", key=f"x{item_id}", use_container_width=True):
                        ...
                        with st.form(key=f"edit_form_{item_id}"):
                            e_name = st.text_input("物品名称", value=it[2], key=f"e_name_{item_id}")
                            e_cate_choice = st.selectbox("类别", CATEGORY_OPTIONS, index=CATEGORY_OPTIONS.index(it[3]) if it[3] in CATEGORY_OPTIONS else 0, key=f"e_cate_{item_id}")
                            if e_cate_choice == "其他":
                                e_cate = st.text_input("具体类别", value=it[3], key=f"e_custom_cate_{item_id}")
                            else:
                                e_cate = e_cate_choice
                            
                            # 处理日期输入，将字符串转回 date 对象
                            try:
                                default_date = datetime.datetime.strptime(it[4], "%Y-%m-%d").date()
                            except:
                                default_date = datetime.date.today()
                            e_time = st.date_input("时间", value=default_date, key=f"e_time_{item_id}")
                            
                            e_loc = st.text_input("地点", value=it[5], key=f"e_loc_{item_id}")
                            e_desc = st.text_area("描述", value=it[6], key=f"e_desc_{item_id}")
                            e_contact = st.text_input("联系方式", value=it[10], key=f"e_con_{item_id}")
                            e_imgs = st.file_uploader("重新上传图片（可选，不传则保留原图）", type=["jpg","png"], accept_multiple_files=True, key=f"e_imgs_{item_id}")
                            
                            form_col1, form_col2 = st.columns(2)
                            with form_col1:
                                if st.form_submit_button("💾 保存修改", use_container_width=True):
                                    edit_item_logic(conn, item_id, e_name, e_cate, e_time, e_loc, e_desc, e_contact, e_imgs)
                                    if "editing_my_item_id" in st.session_state:
                                        del st.session_state["editing_my_item_id"]
                                    st.success("修改成功！")
                                    st.rerun()
                            with form_col2:
        ...
        with t1:
            ...
            if not items:
                ...
                for it in items:
                    render_my_item_card(it, is_active_tab=True)
        ...
        with t2:
            ...
            if not items:
                ...
                for it in items:
                    render_my_item_card(it, is_active_tab=False)
        ...
        with t1:
            ...
            if its:
                ...
                if ex:
                    ...
                    st.warning("⚠️ 由于缺少 openpyxl 库，暂时无法导出 Excel 文件。请联系管理员安装依赖。")
            ...
            if not filtered_its:
                ...
                for it in filtered_its:
                    ...
                    # 检查是否正在编辑此物品
                    editing_admin_item_id = st.session_state.get("editing_admin_item_id")
                    
                    if editing_admin_item_id != item_id:
                        # 非编辑状态：显示简要信息和操作按钮
                        col1, col2, col3 = st.columns([4, 1, 1])
                        col1.write(f"**物品编码**: `{display_no}` | **名称**: {it[idx_name]} | **发布人**: {it[idx_pub]}")
                        
                        if col2.button("✏️ 编辑", key=f"{item_key_prefix}_edit_btn"):
                            st.session_state["editing_admin_item_id"] = item_id
                            st.rerun()
                        ...
                        with st.container(border=True):
                            ...
                            with st.form(key=f"admin_edit_form_{item_id}"):
                                e_name = st.text_input("名称", value=it[idx_name], key=f"{item_key_prefix}_e_name")
                                
                                e_cate_choice = st.selectbox("类别", CATEGORY_OPTIONS, index=CATEGORY_OPTIONS.index(it[idx_cat]) if it[idx_cat] in CATEGORY_OPTIONS else 0, key=f"{item_key_prefix}_e_cate_sel")
                                if e_cate_choice == "其他":
                                    e_cat = st.text_input("具体类别", value=it[idx_cat], key=f"{item_key_prefix}_e_cust_cate")
                                else:
                                    e_cat = e_cate_choice
                                
                                try:
                                    default_date = datetime.datetime.strptime(it[idx_time], "%Y-%m-%d").date()
                                except:
                                    default_date = datetime.date.today()
                                e_time = st.date_input("时间", value=default_date, key=f"{item_key_prefix}_e_time")
                                ...
                                e_imgs = st.file_uploader("重新上传图片（可选）", type=["jpg","png"], accept_multiple_files=True, key=f"{item_key_prefix}_e_imgs")
                                
                                ec1, ec2 = st.columns(2)
                                with ec1:
                                    if st.form_submit_button("💾 保存修改", use_container_width=True):
                                        edit_item_logic(conn, item_id, e_name, e_cat, e_time, e_loc, e_desc, e_con, e_imgs)
                                        if "editing_admin_item_id" in st.session_state:
                                            del st.session_state["editing_admin_item_id"]
                                        st.success("修改成功")
                                        st.rerun()
                                with ec2:
                                    if st.form_submit_button("❌ 取消", use_container_width=True):
        ...
        st.info("💡 提示：请确保Excel包含以下列（顺序不限）：物品名称, 时间, 地点, 类别, 联系方式。第一行可以是表头。")
        f = st.file_uploader("上传Excel", type=["xlsx", "csv"])
        if f:
            try:
                # 尝试读取 Excel，假设第一行可能是表头，先读取所有数据再判断
                if f.name.endswith("xlsx"):
                    df = pd.read_excel(f, header=None)
                else:
                    df = pd.read_csv(f, header=None)
                
                # 简单检查：如果第一行看起来像表头（包含非日期、非数字的常见标题词汇），则跳过
                # 这里为了简化，我们尝试智能识别：如果第一行第一列是字符串且不是典型的数据格式，跳过
                skip_rows = 0
                if not df.empty:
                    first_val = str(df.iloc[0, 0]).strip()
                    # 简单的启发式判断：如果包含“名称”、“名字”、“Item”等字样，视为表头
                    if any(kw in first_val for kw in ["名称", "名字", "Name", "物品", "序号"]):
                        skip_rows = 1
                
                if skip_rows:
                    df = pd.read_excel(f, header=0) if f.name.endswith("xlsx") else pd.read_csv(f, header=0)
                else:
                    # 如果没有表头，手动指定列名以便后续处理，或者按位置读取
                    # 为了兼容旧逻辑，我们这里按位置读取，但增加容错
                    pass
                ...
                if st.button("开始导入", use_container_width=True):
                    ...
                    # 重新读取以进行逐行处理，确保一致性
                    if f.name.endswith("xlsx"):
                        df_process = pd.read_excel(f, header=0) if skip_rows else pd.read_excel(f, header=None)
                    else:
                        df_process = pd.read_csv(f, header=0) if skip_rows else pd.read_csv(f, header=None)
                    ...
                    for index, row in df_process.iterrows():
                        try:
                            # 动态获取列数据，尝试通过列名或索引
                            # 假设标准顺序: 名称(0/名称), 时间(1/时间), 地点(2/地点), 类别(3/类别), 描述(4/描述-可选), 联系(5/联系)
                            
                            # 辅助函数：安全获取值
                            def get_val(idx, name_hint=None):
                                # 如果有表头，尝试用列名
                                if skip_rows and name_hint and name_hint in df_process.columns:
                                    val = row[name_hint]
                                elif idx < len(row):
                                    val = row[idx]
                                else:
                            ...
                            # 尝试多种可能的列名映射
                            name = get_val(0, "物品名称") or get_val(0, "名称") or get_val(0, "Name")
                            time_val = get_val(1, "时间") or get_val(1, "丢失时间") or get_val(1, "Date")
                            loc = get_val(2, "地点") or get_val(2, "位置") or get_val(2, "Location")
                            cate = get_val(3, "类别") or get_val(3, "分类") or get_val(3, "Category")
                            desc = get_val(4, "描述") or get_val(4, "备注") or ""
                            contact = get_val(5, "联系方式") or get_val(5, "电话") or get_val(5, "Contact") or "admin_import"
                            ...
                            itype = "found"
                            if cate:
                                if "丢失" in cate or "lost" in cate.lower():
                                    itype = "lost"
                                elif "招领" in cate or "found" in cate.lower():
                                    itype = "found"
                                else:
                                    # 默认根据上下文或用户选择？这里默认设为 lost，或者可以根据名称判断
                                    # 为了安全，如果无法判断，默认设为 lost 或让用户后续修改
                                    itype = "lost" 
                            else:
                                itype = "lost"
                            ...
                            c.execute('''INSERT INTO items 
                                      (item_no, type, name, category, time, location, description, image, ai_result, publisher, contact, create_time, status, is_hidden) 
                                      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                                      (new_item_no, itype, name, cate if cate else "其他", str(time_val), loc, desc if desc else "", None, "批量导入", "admin", contact, str(datetime.datetime.now()), 0, 0))
                            cnt += 1
                        except Exception as e:
                            err_cnt += 1
                            # 可以在调试时打开下面这行查看具体错误
                            # st.warning(f"行 {index} 导入失败: {str(e)}")
                            continue
                    
                    conn.commit()
                    if cnt > 0:
                        st.success(f"✅ 成功导入 {cnt} 条数据！")
                    if err_cnt > 0:
                        st.warning(f"⚠️ 有 {err_cnt} 条数据因格式问题被跳过。")
                    
            except Exception as e:
                st.error(f"❌ 文件解析失败: {str(e)}")
                st.info("请检查Excel格式是否正确，建议第一行为表头。")
